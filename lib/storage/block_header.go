...
type blockHeader struct {
	// TraceID is the TraceID for the block.
	// Multiple blocks may have the same TraceID.
	TraceID TraceID
...
func (bh *blockHeader) Marshal(dst []byte) []byte {
	...
	dst = append(dst, byte(bh.TimestampsMarshalType), byte(bh.ValuesMarshalType))
	return dst
}
...
func (bh *blockHeader) Unmarshal(src []byte) ([]byte, error) {
	...
	tail, err := bh.TraceID.Unmarshal(src)
	if err != nil {
		return src, fmt.Errorf("cannot unmarshal TraceID: %w", err)
	}
	src = tail
...
//
// Block headers must be sorted by bh.TraceID.
