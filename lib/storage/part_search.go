...
func (ps *partSearch) Init(p *part, tsids []TraceID, tr ScanRange) {
	ps.reset()
	ps.p = p
...
// for the same TraceID may contain overlapped time ranges.
...
func (ps *partSearch) nextTSID() bool {
	...
	ps.BlockRef.bh.TraceID = ps.tsids[ps.tsidIdx]
	ps.tsidIdx++
	return true
}
...
func (ps *partSearch) nextBHS() bool {
	for len(ps.metaindex) > 0 {
		...
		// Invariant: ps.BlockRef.bh.TraceID >= ps.metaindex[0].TraceID

		ps.metaindex = skipSmallMetaindexRows(ps.metaindex, ps.BlockRef.bh.TraceID)
		// Invariant: len(ps.metaindex) > 0 && ps.BlockRef.bh.TraceID >= ps.metaindex[0].TraceID
		...
		// for the ps.BlockRef.bh.TraceID and the given timestamp range.
...
func (ps *partSearch) searchBHS() bool {
	...
	nextTSID:
		...
		// While blocks for the same TraceID are sorted by MinTimestamp,
