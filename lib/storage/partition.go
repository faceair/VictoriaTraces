...
func createPartition(timestamp int64, smallPartitionsPath, bigPartitionsPath string, retentionMsecs int64) (*partition, error) {
	...
	pt := newPartition(name, smallPartsPath, bigPartsPath, retentionMsecs)
...
func openPartition(smallPartsPath, bigPartsPath string, retentionMsecs int64) (*partition, error) {
...
func newPartition(name, smallPartsPath, bigPartsPath string, retentionMsecs int64) *partition {
	p := &partition{
		name:           name,
...
func (pt *partition) mergeParts(pws []*partWrapper, stopCh <-chan struct{}) error {
	...
	err := mergeBlockStreams(&ph, bsw, bsrs, stopCh, retentionDeadline, rowsMerged, rowsDeleted)
	if isBigPart {
