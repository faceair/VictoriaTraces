...
func (mn *SpanName) CopyFrom(src *SpanName) {
	if cap(mn.TraceID) > 0 {
		mn.TraceID = append(mn.TraceID[:0], src.TraceID...)
		mn.Tags = copyTags(mn.Tags[:0], src.Tags)
		return
	}

	// Pre-allocate a single byte slice for TraceID + all the tags.
	// This reduces the number of memory allocations for zero mn.
	size := len(src.TraceID)
	...
	b = append(b, src.TraceID...)
	mn.TraceID = b[:len(b):len(b)]
...
func (mn *SpanName) addNextTag() *Tag {
...
func (mn *SpanName) RemoveTagsIgnoring(ignoringTags []string) {
	...
	if hasTag(ignoringTags, spanGroupTagKey) {
		mn.ResetMetricGroup()
	}
	tags := mn.Tags
	mn.Tags = mn.Tags[:0]
...
func (mn *SpanName) SetTags(addTags []string, src *SpanName) {
	for _, tagName := range addTags {
		if tagName == string(spanGroupTagKey) {
			mn.TraceID = append(mn.TraceID[:0], src.TraceID...)
			continue
		}
		var srcTag *Tag
		...
		if !found {
			mn.AddTag(srcTag.Key, srcTag.Value)
		}
	}
}
...
func (mn *SpanName) String() string {
	...
	return fmt.Sprintf("traceid=%s, tags=[%s]", mn.TraceID, tagsStr)
}
...
func (mn *SpanName) Marshal(dst []byte) []byte {
	...
	dstLen := len(dst)
	requiredSize := 8 + len(mn.TraceID) + 1
	for i := range mn.Tags {
		tag := &mn.Tags[i]
	...
	dst = marshalTagValue(dst, mn.TraceID)
...
func (mn *SpanName) Unmarshal(src []byte) error {
	...
	// Unmarshal TraceID.
	var err error
	src, mn.TraceID, err = unmarshalTagValue(mn.TraceID[:0], src)
	if err != nil {
		return fmt.Errorf("cannot unmarshal TraceID: %w", err)
	}
	...
	// since they must be handled in SpanName.Marshal inside marshalTags.
...
//
// The results may be unmarshaled with SpanName.unmarshalRaw.
//
// This function is for testing purposes. MarshalMetricNameRaw must be used
// in prod instead.
func (mn *SpanName) marshalRaw(dst []byte) []byte {
	dst = marshalBytesFast(dst, nil)
	dst = marshalBytesFast(dst, mn.TraceID)
...
func (mn *SpanName) unmarshalTraceID(src []byte) ([]byte, error) {
	src, key, err := unmarshalBytesFast(src)
	if err != nil {
		return nil, err
	}
	if len(key) > 0 {
		return nil, fmt.Errorf("cannot unmarshal TraceID: %s", key)
	}
	src, traceID, err := unmarshalBytesFast(src)
	if err != nil {
		return nil, err
	}
	mn.TraceID = traceID
...
func (mn *SpanName) unmarshalRaw(src []byte) error {
	mn.Reset()
	for len(src) > 0 {
		...
		if len(key) == 0 {
			mn.TraceID = append(mn.TraceID[:0], value...)
		} else {
			mn.AddTag(key, value)
		}
	}
...
var commonTagKeys = func() map[string][]byte {
	lcm := map[string][]byte{
		// job-like tags must go first in SpanName.Labels.
		...
		// instance-like tags must go second in SpanName.Labels.
		// This should improve data locality.
