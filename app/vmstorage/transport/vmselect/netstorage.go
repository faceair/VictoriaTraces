...
func RegisterMetricNames(mrs []storage.SpanRow, deadline searchutils.Deadline) error {
	// Split mrs among available vmstorage nodes.
	mrsPerNode := make([][]storage.SpanRow, len(storageNodes))
	for _, mr := range mrs {
		idx := 0
...
// GetLabelsOnTimeRange returns labels for the given sr until the given deadline.
func GetLabelsOnTimeRange(denyPartialResponse bool, tr storage.ScanRange, deadline searchutils.Deadline) ([]string, bool, error) {
	...
	snr := startStorageNodesRequest(denyPartialResponse, func(idx int, sn *storageNode) interface{} {
		...
		labels, err := sn.getLabelsOnTimeRange(tr, deadline)
		if err != nil {
...
func GetLabels(denyPartialResponse bool, deadline searchutils.Deadline) ([]string, bool, error) {
	if deadline.Exceeded() {
		return nil, false, fmt.Errorf("timeout exceeded before starting the query processing: %s", deadline.String())
	}
	// Send the query to all the storage nodes in parallel.
	...
	snr := startStorageNodesRequest(denyPartialResponse, func(idx int, sn *storageNode) interface{} {
		...
		labels, err := sn.getLabels(deadline)
		if err != nil {
			sn.labelsErrors.Inc()
...
// GetLabelValuesOnTimeRange returns label values for the given labelName on the given sr
// until the given deadline.
func GetLabelValuesOnTimeRange(denyPartialResponse bool, labelName string, sr storage.ScanRange, deadline searchutils.Deadline) ([]string, bool, error) {
	if deadline.Exceeded() {
		return nil, false, fmt.Errorf("timeout exceeded before starting the query processing: %s", deadline.String())
	}
	...
	snr := startStorageNodesRequest(denyPartialResponse, func(idx int, sn *storageNode) interface{} {
		...
		labelValues, err := sn.getLabelValuesOnTimeRange(labelName, sr, deadline)
		if err != nil {
...
func GetLabelEntries(denyPartialResponse bool, deadline searchutils.Deadline) ([]storage.TagEntry, bool, error) {
	if deadline.Exceeded() {
		return nil, false, fmt.Errorf("timeout exceeded before starting the query processing: %s", deadline.String())
	}
	// Send the query to all the storage nodes in parallel.
	...
	snr := startStorageNodesRequest(denyPartialResponse, func(idx int, sn *storageNode) interface{} {
		...
		labelEntries, err := sn.getLabelEntries(deadline)
		if err != nil {
...
func GetSeriesCount(denyPartialResponse bool, deadline searchutils.Deadline) (uint64, bool, error) {
	if deadline.Exceeded() {
	...
	snr := startStorageNodesRequest(denyPartialResponse, func(idx int, sn *storageNode) interface{} {
		...
		n, err := sn.getSeriesCount(deadline)
		if err != nil {
...
func (sn *storageNode) registerMetricNames(mrs []storage.SpanRow, deadline searchutils.Deadline) error {
	if len(mrs) == 0 {
		return nil
	}
...
func (sn *storageNode) getLabelsOnTimeRange(tr storage.ScanRange, deadline searchutils.Deadline) ([]string, error) {
	var labels []string
	f := func(bc *handshake.BufferedConn) error {
		ls, err := sn.getLabelsOnTimeRangeOnConn(bc, tr)
		if err != nil {
			return err
		}
		labels = ls
...
func (sn *storageNode) getLabels(deadline searchutils.Deadline) ([]string, error) {
	var labels []string
	f := func(bc *handshake.BufferedConn) error {
		ls, err := sn.getLabelsOnConn(bc)
		if err != nil {
			return err
		}
		labels = ls
...
func (sn *storageNode) getLabelValuesOnTimeRange(labelName string, sr storage.ScanRange, deadline searchutils.Deadline) ([]string, error) {
	var labelValues []string
	f := func(bc *handshake.BufferedConn) error {
		lvs, err := sn.getLabelValuesOnTimeRangeOnConn(bc, labelName, sr)
		if err != nil {
			return err
		}
		labelValues = lvs
...
func (sn *storageNode) getLabelEntries(deadline searchutils.Deadline) ([]storage.TagEntry, error) {
	var tagEntries []storage.TagEntry
	f := func(bc *handshake.BufferedConn) error {
		tes, err := sn.getLabelEntriesOnConn(bc)
		if err != nil {
			return err
		}
		tagEntries = tes
...
func (sn *storageNode) getSeriesCount(deadline searchutils.Deadline) (uint64, error) {
	var n uint64
	f := func(bc *handshake.BufferedConn) error {
		nn, err := sn.getSeriesCountOnConn(bc)
		if err != nil {
			return err
		}
		n = nn
...
func (sn *storageNode) registerMetricNamesOnConn(bc *handshake.BufferedConn, mrs []storage.SpanRow) error {
	// Send the request to sn.
	if err := writeUint64(bc, uint64(len(mrs))); err != nil {
		return fmt.Errorf("cannot send metricsCount to conn: %w", err)
	}
	for i, mr := range mrs {
		if err := writeBytes(bc, mr.SpanNameRaw); err != nil {
...
func (sn *storageNode) getLabelsOnTimeRangeOnConn(bc *handshake.BufferedConn, tr storage.ScanRange) ([]string, error) {
	// Send the request to sn.
